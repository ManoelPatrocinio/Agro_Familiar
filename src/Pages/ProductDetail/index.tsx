      ...
      <div className="mt-6 ">
        ...
        {commentsList.length === 0 ? (
          <h4 className="w-full text-left text-sm md:text-md text-gray-500 md:pl-6 mb-20">
            Este produto ainda não tem comentários e avaliações,{" "}
            {!isAuthenticated && "logue com sua conta e "} seja o primeiro a
            comentar 😀
          </h4>
        ) : (
  ...
  return (
    <div className="w-full h-full">
      ...
      <main className="w-full h-full md:h-[20rem] md:max-h-screen  flex flex-col md:flex-row px-4 lg:px-20 mb-10">
        <div className="relative w-full h-full max-h-screen  md:w-1/2   ">
            ...
            <div className="w-full h-full flex justify-center items-start space-x-2">
              {productData?.p_images?.map((url, index) => (
                <button
                  ...
                  className="w-[6rem] h-[6rem] md:w-1/4 md:h-full rounded cursor-pointer bg-black hover:border hover:border-palm-700 "
                  key={index}
                >
                  <img
                    src={url}
                    alt="foto"
                    className="w-full h-full object-cover rounded hover:opacity-50 smallImg"
                  />
                </button>
              ))}
            </div>
        ...
        <div className="w-full md:w-1/2 h-auto flex flex-col justify-between items-start  pt-4 md:px-4 md:pt-0 lg:pl-8 lg:pr-20">
            ...
            <div className="w-full flex flex-col md:flex-row justify-center md:justify-between items-center">
              ...
              <div className="flex justify-start items-center">
                ...
                <div className=" flex items-center space-x-4">
                  {productData?.p_payments?.includes("money") && (
                    <img
                      src={Icon_money}
                      alt="dinheiro"
                      className="w-8 h-8"
                  ...
                  {productData?.p_payments?.includes("pix") && (
                    <img
                      src={Icon_pix}
                      alt="pix"
                      className="w-10 h-8"
                  ...
                  {productData?.p_payments?.includes("cart") && (
                    <img
                      src={Icon_cart}
                      alt="cartão"
                      className="w-7 h-7"
          ...
          <div className="w-full max-h-2/3">
            <p className="w-full min-h-[20%] text-justify md:text-start text-xs md:text-sm text-gray-400 mb-3">
              ...
              negociação e entrega, deve ser feito entre o cliente e responsável
              por este produto.
            </p>
            ...
            <a
              href={`http://api.whatsapp.com/send?l=pt_BR&phone=+55${productData?.p_n_contact}&text=Olá tudo bem ? Eu Tenho interesse no produto: ${productData?.p_name}, de preço: ${productData?.p_price} reais. Ainda estar disponível ?`}
              target="_blank"
